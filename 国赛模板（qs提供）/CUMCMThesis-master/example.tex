...
innertopmargin=\topskip, frametitle={2026 年建模比赛格式变化说明},
...
今年的格式变化主要就是一个地方，如下：
\begin{enumerate}
\item 浙江省赛区一般第一页是论文摘要页，无须前面的两页（承诺书和专用页）。论文承诺书会在比赛区间找大家确认并签字的。\textbf{\color{red}内容进行了调整}。
\item 论文明确无须目录，正文长度不超过30页，整个pdf文件的大小不超过20MB。
