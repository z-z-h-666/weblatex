...
\begin{figure}[H]
	\centering
	\IfFileExists{1.png}{\includegraphics[width=0.65\textwidth]{1.png}}{\fbox{Missing figure}}
	\caption{飞机飞行状态示意图}
...
将式 (32) 代入式 (34)
和式 (35)，即可构成关于 $m(t)$ 的一阶微分方程。
数值求解该方程后，再由式 (30) 得到等速巡航爬升策略下的高度剖面。
...
式 (40)即为等马赫数巡航爬升策略下高度与质量之间的隐式关系。
由于温度 $T(h)$ 也是高度的函数，因此该式一般难以直接显式解出 $h(t)$，
可在数值计算中通过一维方程求根获得高度剖面。
...
\begin{figure}[H]
	\centering
	\IfFileExists{2.png}{\includegraphics[width=1.1\textwidth]{2.png}}{\fbox{Missing figure}}
	\caption{飞机飞行状态示意图}
	\label{fig:pipe1_comparison}
\end{figure}
